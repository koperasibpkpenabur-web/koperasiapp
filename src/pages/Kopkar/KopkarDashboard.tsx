...
const KopkarDashboard = () => {
  ...
  return (
    <div className="kopkar-dashboard">
      <h2>Kopkar Area — Pengelolaan Pesanan & Pelunasan Fee Sekolah</h2>
      ...
      <div className="dashboard-tabs">
        ...
        >
          ✅ History Diterima & Pelunasan ({receivedCount})
        </button>
        <button
          className={`tab-button ${activeTab === 'cancellations' ? 'active' : ''}`}
          onClick={() => setActiveTab('cancellations')}
        >
          🚫 History Pembatalan ({cancellationOrders.length})
          {cancelRequestsCount > 0 && <span className="tab-pill">{cancelRequestsCount}</span>}
        </button>
      </div>
